...
assert.match(
  indexSource,
  /styles\.css\?v=1\.19-release[\s\S]*app\.js\?v=1\.19-release/,
  "1.19 release busts cached app and style assets"
);
...
assert.match(
  appSource,
  /function\s+renderFlightHistoryTimeline\(detail\)[\s\S]*const\s+filteredFlights\s*=\s*historyFlightsForTimeline\(detail\);[\s\S]*const\s+historyNow\s*=\s*historyTimelineNowEpoch\(detail\);[\s\S]*const\s+summary\s*=\s*historySummary\(detail,\s*filteredFlights\);[\s\S]*renderHistoryCurrentStatus\(detail,\s*filteredFlights,\s*selectedAircraft\(\),\s*historyNow\)[\s\S]*renderHistoryControls\(summary\)[\s\S]*renderHistoryActivityBar\(filteredFlights,\s*historyNow\)[\s\S]*renderHistoryTimelineGroups\(filteredFlights\)/,
  "journey tab renders current status, period controls, summary, activity bars, and the table list in the v1.24 order"
);
...
assert.doesNotMatch(
  appSource,
  /data-history-range="90"|>90d<|最近飞行日期|常飞机场|年度活动|近 30 天|history-filter-row|data-history-status|historyAirportFilter/,
...
assert.match(
  appSource,
  /function\s+renderHistoryTimelineGroups\(flights\)[\s\S]*const\s+currentFlightKey\s*=\s*historyFlightKey\(historyCurrentLiveFlight\(null,\s*flights\)\);[\s\S]*<div class="history-list-head"[\s\S]*日期[\s\S]*航线[\s\S]*时长[\s\S]*history-flight-table[\s\S]*history-month-table-group[\s\S]*group\.flights\.map\(\(item\)\s*=>\s*renderHistoryFlightTableRow\(item,\s*\{\s*current:/,
  "history list renders the fixed-header large-screen table layout grouped by sticky months"
);

assert.match(
  appSource,
  /function\s+handleHistoryTimelineClick\(event\)[\s\S]*data-history-airport[\s\S]*selectAirportFromCode\(code\)[\s\S]*data-history-range[\s\S]*data-history-flight-card[\s\S]*action\s*===\s*"live"[\s\S]*setAircraftMapMode\("follow"\)/,
  "history interactions support airport handoff, period reset, row expansion, and Live follow"
);

assert.match(
  appSource,
  /function\s+resetHistoryScrollTop\(options\s*=\s*{}\)[\s\S]*scrollHistoryToCurrentStatus\(scroller\);[\s\S]*window\.requestAnimationFrame\(\(\)\s*=>\s*{[\s\S]*window\.requestAnimationFrame\(\(\)\s*=>\s*{[\s\S]*scrollTop\(\);[\s\S]*window\.setTimeout\(scrollTop,\s*80\);[\s\S]*window\.setTimeout\(scrollTop,\s*180\);[\s\S]*window\.setTimeout\(scrollTop,\s*360\);[\s\S]*window\.setTimeout\(scrollTop,\s*720\);/,
  "journey tab initial reset forcefully pins the detail scroller to the current status or live flight card across late layout shifts"
);
...
assert.match(
  appSource,
  /function\s+historyCurrentStatusScrollTop\(scroller\s*=\s*historyDetailScroller\(\)\)[\s\S]*querySelector\("\.history-current-row"\)[\s\S]*querySelector\("\.history-current-block"\)[\s\S]*targetRect\.top\s*-\s*scrollerRect\.top\s*-\s*8[\s\S]*function\s+scrollHistoryToCurrentStatus\(scroller\s*=\s*historyDetailScroller\(\)\)[\s\S]*scroller\.scrollTop\s*=\s*historyCurrentStatusScrollTop\(scroller\);/,
  "journey reset anchors to the live flight row first and falls back to the current status card"
);

assert.match(
  appSource,
  /function\s+restoreHistoryScrollState\(snapshot\)[\s\S]*const\s+restoreTop\s*=\s*\(nextTop\)\s*=>[\s\S]*snapshot\.forceTop[\s\S]*restoreTop\(historyCurrentStatusScrollTop\(scroller\)\)[\s\S]*if\s*\(historyScrollResetIsActive\(\)\)\s*{[\s\S]*restoreTop\(historyCurrentStatusScrollTop\(scroller\)\);/,
  "all rerenders during the initial journey reset ignore captured bottom offsets and return to the current status anchor"
);
...
assert.match(
  appSource,
  /function\s+historyScrollResetShouldPin\(token\)[\s\S]*historyScrollResetIsActive\(token\)[\s\S]*expectedScrollTop\s*=\s*historyCurrentStatusScrollTop\(scroller\);[\s\S]*hasMovedAwayFromCurrentStatus\s*=\s*Math\.abs\(Number\(scroller\?\.scrollTop\s*\|\|\s*0\)\s*-\s*expectedScrollTop\)\s*>\s*4;[\s\S]*releaseHistoryScrollReset\(token\);[\s\S]*return\s+false;/,
  "late reset calls are cancelled when the user has already moved the journey list away from the current status anchor"
);
...
assert.match(
  appSource,
  /function\s+setRecentFlightsHtml\(html,\s*options\s*=\s*{}\)[\s\S]*const\s+staleResetScroll\s*=\s*options\.resetScroll\s*===\s*true[\s\S]*!historyScrollResetShouldPin\(options\.resetToken\);[\s\S]*const\s+effectiveOptions\s*=\s*staleResetScroll[\s\S]*preserveScroll:\s*true[\s\S]*const\s+restoreSeq\s*=\s*nextHistoryScrollRestoreSeq\(\);[\s\S]*captureHistoryScrollState\(effectiveOptions,\s*restoreSeq\)/,
  "each history rerender receives a fresh scroll restore sequence"
);
...
assert.match(
  appSource,
  /function\s+setRecentFlightsHtml\(html,\s*options\s*=\s*{}\)[\s\S]*const\s+htmlChanged\s*=\s*timeline\.recentFlightsHtml\s*!==\s*nextHtml[\s\S]*if\s*\(htmlChanged\)\s*{[\s\S]*element\.innerHTML\s*=\s*nextHtml[\s\S]*timeline\.recentFlightsHtml\s*=\s*nextHtml[\s\S]*if\s*\(htmlChanged\)\s*{[\s\S]*restoreHistoryScrollState\(snapshot\);/,
  "history list skips identical HTML rerenders and only restores scroll after real DOM changes"
);
...
assert.match(
  appSource,
  /activeUserScroll:\s*Date\.now\(\)\s*-\s*Number\(timeline\.lastUserScrollAt\s*\|\|\s*0\)\s*<\s*2500[\s\S]*if\s*\(Number\.isFinite\(Number\(snapshot\.scrollTop\)\)\)\s*{[\s\S]*restoreTop\(snapshot\.scrollTop\);/,
  "history rerenders preserve the exact scroll offset instead of anchor-snapping upward"
);
...
assert.match(
  ...
  "switching aircraft while the journey tab is active opens the next aircraft at the current status card without resetting realtime refreshes of the same aircraft"
);

assert.match(
  appSource,
  /function\s+handleHistoryTimelineScroll\(event\)[\s\S]*if\s*\(historyScrollResetIsActive\(\)\)\s*{[\s\S]*expectedScrollTop\s*=\s*historyCurrentStatusScrollTop\(scroller\);[\s\S]*hasMovedAwayFromCurrentStatus[\s\S]*const\s+hardLocked\s*=\s*Date\.now\(\)\s*<\s*Number\(timeline\.scrollResetHardUntil\s*\|\|\s*0\);[\s\S]*timeline\.applyingScrollRestore\s*\|\|\s*hardLocked[\s\S]*scrollHistoryToCurrentStatus\(scroller\);[\s\S]*else\s*{[\s\S]*timeline\.lastUserScrollAt\s*=\s*Date\.now\(\);[\s\S]*nextHistoryScrollRestoreSeq\(\);[\s\S]*releaseHistoryScrollReset\(\);[\s\S]*return;[\s\S]*!timeline\.applyingScrollRestore\s*&&\s*!historyScrollRestoreIsPending\(\)[\s\S]*timeline\.lastUserScrollAt\s*=\s*Date\.now\(\);[\s\S]*nextHistoryScrollRestoreSeq\(\);[\s\S]*syncHistoryCollapsedState\(scroller\);/,
  "scroll events during the short initial reset window are corrected, while later real scroll cancels the current status reset lock"
);

assert.match(
  appSource,
  /function\s+handleHistoryScrollIntent\(\)[\s\S]*state\.aircraftSegment\s*!==\s*"journey"[\s\S]*historyTimelineState\(\)\.lastUserScrollAt\s*=\s*Date\.now\(\);[\s\S]*nextHistoryScrollRestoreSeq\(\);[\s\S]*historyScrollResetIsActive\(\)[\s\S]*releaseHistoryScrollReset\(\);/,
  "explicit user scroll intent cancels pending restores and the journey top reset lock"
);

assert.match(
  appSource,
  /aircraftDetailScroller\?\.addEventListener\("scroll",\s*handleHistoryTimelineScroll\);[\s\S]*aircraftDetailScroller\?\.addEventListener\("wheel",\s*handleHistoryScrollIntent,\s*{\s*passive:\s*true\s*}\);[\s\S]*aircraftDetailScroller\?\.addEventListener\("touchmove",\s*handleHistoryScrollIntent,\s*{\s*passive:\s*true\s*}\);[\s\S]*aircraftDetailScroller\?\.addEventListener\("keydown",\s*handleHistoryScrollIntent\);/,
  "detail scroller listens for wheel, touch, and keyboard intent before allowing manual scroll"
);
...
assert.match(
  stylesSource,
  /\.selected-panel-v114\s+\.history-list-head\s*{[\s\S]*grid-template-columns:\s*56px\s+minmax\(0,\s*1fr\)\s+80px;[\s\S]*column-gap:\s*14px;/,
  "history list header gives the date and route columns more breathing room"
);
...
assert.match(
  appSource,
  /const\s+isMonth\s*=\s*!isYear\s*&&\s*rangeDays\s*>\s*7;[\s\S]*history-activity-axis-dot[\s\S]*bucket\.secondaryLabel/,
...
assert.match(
  stylesSource,
  /\.selected-panel-v114\s+\.history-activity-bars button::after\s*{[\s\S]*content:\s*attr\(data-history-activity-tooltip\);[\s\S]*opacity:\s*0;[\s\S]*\.history-activity-bars button:hover::after,[\s\S]*opacity:\s*1;/,
  "activity columns reveal concrete data on hover"
);
