...
> **1.8 实施状态（2026-08-04）：** 根目录版本已落地 selected track store。`513009` 历史航迹与 `513008` 实时快照尾迹会按时间合并、去重和增量追加；`120s` 以上覆盖缺口、`600s` 以上长缺口、时间顺序异常或隐含速度异常，只要两端坐标有效，都会按相邻航路点顺序显示为 estimated/interrupted gap 点线，不再留空。若当前航线目的地机场坐标已知，地图会从飞机当前实时位置绘制一条独立 planned 虚线到目的地；目的地未知时不绘制。实现入口见 `app.js` 的 `selectedTrackStore`、`appendSelectedRealtimeTrackPoint()`、`selectedPlannedRoutePath()`、`trackEstimationReason()` 和 `trackBreakReason()`。
...
4. 航迹视觉继续遵守 FR24 风格：实际航迹按高度色阶，估算/覆盖缺口/中断补接使用深色点线或弱化线，不用 selected 颜色覆盖高度语义。
...
| 途中状态 | `state=30` 时，已飞段为实线，最后点到目的地为未完成虚线 | 1.8 起目的地已知时显示独立 planned layer；目的地未知时不显示 |
...
以下场景绘制为 estimated/coverage gap/interrupted gap：

- 后端显式标记 `isEstimated`、`estimatedToNext` 或 `quality=estimated`；
- `elapsedMs > 120000` 的覆盖缺口或长缺口；
- 覆盖缺失但起终点仍可通过合理速度解释；
- `513009` 历史最后点与 `513008` 当前点之间存在短暂缺口。
- `elapsedMs > 600000` 的长缺口，只要两端坐标有效，仍按相邻航路点顺序绘制虚线；
- 两端坐标有效但隐含速度超过阈值、时间戳相同或时间戳倒序时，绘制为 `interrupted_gap` 虚线，不作为 actual segment。
...
无效点断开时保留两侧独立轨迹段。除此之外，所有相邻有效航路点之间都应至少以 estimated/interrupted gap 虚线连接，避免 selected 航迹出现可见空白断口。

### 7.4 机场连接与未来计划线

selected 航迹需要区分已飞路径和未来连接线：

- 起飞机场到首个实测点的连接线默认关闭，可在 Route focus 中以弱化虚线显示。
- 当前点到目的地的未来计划线在目的地机场坐标已知时默认显示。
- 目的地未知、无机场坐标或当前实时位置无效时，不绘制未来计划线。
- 未来计划线必须使用 planned route layer，样式为浅灰/浅黄长虚线，层级低于 actual/estimated。
...
4. 增加自动化用例覆盖新增点追加、缺口点线、无效点断开、日期变更线。
