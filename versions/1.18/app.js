...
function desiredAirportLabelMode(airport) {
  ...
  const scale = effectiveScaleKm();
  if (Number.isFinite(scale) && scale <= mapLoadingConfig.airportDetailLabelScaleKm) {
    return "full";
  }
  if (Number.isFinite(scale) && scale <= mapLoadingConfig.airportCodeLabelScaleKm) {
    return "code";
  }
  return "none";
}
...
function airportMarkerMetrics(airport) {
  ...
  if ((selected || routeEndpoint) && (!base.width || !base.height)) {
...
function renderAirports() {
  ...
  airportLayer.innerHTML = airportMarkers.map((airport) => {
    ...
    return `
      <button type="button" class="${airportMarkerClass(airport, metrics)}${hoveredClass}${popupReadyClass}${currentHoverClass}" data-id="${airport.id}" data-level="${airportPriorityLevel(airport)}" data-popup-placement="${popupVars.placement}"${currentHoverAttr}${routeEndpointAttr} style="left:${point.x}px; top:${point.y}px; ${cssText} ${popupCssText}" aria-label="${escapeHtml(airportFullLabel(airport))}">
        <span class="airport-marker-hit">
          <span class="marker-map-shadow airport-map-shadow" aria-hidden="true"></span>
          <svg class="airport-pin-icon" viewBox="0 0 28 36" aria-hidden="true">
            <path class="airport-pin-body" d="M14 1.8C7.1 1.8 2.3 6.8 2.3 13.2c0 7.8 9.2 19 11 21a.95.95 0 0 0 1.4 0c1.8-2 11-13.2 11-21C25.7 6.8 20.9 1.8 14 1.8Z"></path>
...
function airportsForCurrentView() {
  ...
  const inView = airports
    ...
    .filter((item) => item.protected || item.level <= maxLevel)
...
async function loadAircraftHistory(jet) {
  if (!dataService?.isEnabled() || !jet || !jet.tailNoEncrypted || jet.flightHistoryDetail) {
    return;
  }
  const loadKey = `aircraft-history:${jet.tailNoEncrypted}`;
  if (state.detailLoads.has(loadKey)) {
    return;
  }
  state.detailLoads.add(loadKey);
  try {
    const detail = await dataService.getFlightHistory(jet.tailNoEncrypted);
    const currentJet = aircraftByDetailSeed(jet);
    if (!currentJet || !detail) {
      return;
    }
    applyAircraftHistory(currentJet, detail);
    if (state.selectedKind === "aircraft" && state.selectedId === currentJet.id) {
      renderAircraftDetailPanel(currentJet);
    }
    renderApiDebugConsole();
    ...
    if (currentJet) {
      ...
      if (state.selectedKind === "aircraft" && state.selectedId === currentJet.id) {
        renderRecentFlights(currentJet);
      }
      renderApiDebugConsole();
    }
  } finally {
...
async function loadAirportGround(airport) {
  ...
  try {
    ...
    if (currentAirport) {
      ...
      if (state.selectedKind === "airport" && state.selectedId === currentAirport.id) {
        renderAirportTabList(currentAirport);
      }
      renderApiDebugConsole();
    }
  } finally {
...
function setAircraftSegment(segment, options = {}) {
  ...
  if (nextSegment === "journey") {
    loadAircraftHistory(selectedAircraft());
  }
}
...
function setAirportSegment(segment) {
  ...
  if (airport) {
    loadAirportDetail(airport);
    if (state.airportSegment === "dynamic" && state.airportTab === "ground") {
    ...
    return [
      {
        pid: "513008",
        title: "实时在途飞机条目",
        request: { selectedUniqueKey: jet.uniqueKey || jet.id, tailNo: jet.tailNoEncrypted || "" },
        raw: jet.raw || null,
      ...
      },
      {
        pid: "513011",
        title: "飞机基础信息",
        request: { tailNo: jet.tailNoEncrypted || "" },
...
function aircraftHistoryLoadKey(jet) {
  return jet?.tailNoEncrypted ? `aircraft-history:${jet.tailNoEncrypted}` : "";
}

function formatHistoryTime(timeRef) {
  if (!timeRef || !timeRef.epochMs) {
    return NA_TEXT;
  }
  return formatPanelTime(timeRef, {
    date: true,
    timeZone: timeRef.displayZone || "UTC",
    includeZone: false,
    rawUnknown: false
  });
}
...
function renderFlightHistoryRows(detail) {
  ...
  const groundSummary = ground
    ? `<div class="journey-ground-summary">
        <span>当前停场</span>
        <strong>${escapeHtml(displayOrDash(firstMatchedValue(ground.airportFourName, ground.airportName, ground.airportCode)))}</strong>
        <small>${escapeHtml([ground.airportCode, ground.airportNameEn].filter((value) => !missingValue(value)).join(" | "))}</small>
      </div>`
    : "";
  const rows = (detail?.flights || []).slice(0, 6).map((item) => {
    const dep = displayOrDash(item.depAirport);
    const arr = displayOrDash(item.arrAirport);
    const callSign = firstMatchedValue(item.callSign, item.flightNo);
    const stateText = firstMatchedValue(item.flightStateStr, item.flightState === 30 ? "途中" : "");
    const duration = item.estimateTimeMinutes ? formatDuration(Number(item.estimateTimeMinutes) * 60000) : NA_TEXT;
    const depName = firstMatchedValue(item.depAirportName, item.depAirportFullName, item.depAirportNameEn);
    const arrName = firstMatchedValue(item.arrAirportName, item.arrAirportFullName, item.arrAirportNameEn);
    return `
      <button type="button" class="related-flight history-flight-row" data-history-id="${escapeHtml(item.uniqueKey || item.flightId || "")}">
        <span>
          <strong>${escapeHtml(firstMatchedValue(callSign, `${dep} - ${arr}`))}</strong>
          <small>${escapeHtml(`${dep} - ${arr}`)}</small>
          <small>${escapeHtml([depName, arrName].filter((value) => !missingValue(value)).join(" → "))}</small>
          <small>${escapeHtml([formatHistoryTime(item.depTimeRef), formatHistoryTime(item.arrTimeRef)].filter((value) => value !== NA_TEXT).join(" / "))}</small>
        </span>
        <span class="related-flight-meta">${escapeHtml(firstMatchedValue(stateText, duration))}</span>
      </button>`;
  }).join("");
  return `${groundSummary}${rows}`;
}

function renderRecentFlights(jet) {
  const historyDetail = jet.flightHistoryDetail || cachedAircraftHistoryDetail(jet);
  if (historyDetail?.flights?.length || historyDetail?.groundAirportInfo) {
    setHtml("recentFlightsList", renderFlightHistoryRows(historyDetail));
    return;
  }
  if (state.detailLoads.has(aircraftHistoryLoadKey(jet))) {
    setHtml("recentFlightsList", `<p class="empty-related">行程记录加载中...</p>`);
    return;
  }
  if (jet.flightHistoryError) {
    setHtml("recentFlightsList", `<p class="empty-related">行程记录暂时无法加载</p>`);
    return;
  }
...
function renderAircraftMoreMenu(jet) {
  ...
  const copyCallsign = aircraftCallsignLabel(jet);
  ...
  menu.innerHTML = `
    ...
    <button type="button" class="more-row" data-copy-value="${escapeHtml(copyCallsign === NA_TEXT ? "" : copyCallsign)}">
      <span>复制航班号</span><strong>${escapeHtml(copyCallsign)}</strong>
    </button>
...
function renderAircraftDetailPanel(jet) {
  ...
  setText("aircraftCallsign", aircraftCallsignLabel(jet, "暂无航班号"));
...
function airportGroundModels(airport) {
  return airport?.apiGround?.groundModels
    || airport?.apiGround?.groundInfo?.groundModels
    || [];
}
...
function createPanelAircraftFromGroundPlane(airport, plane = {}) {
  const registration = firstMatchedValue(plane.registration, plane.tailNoDisplay);
  ...
  Object.assign(jet, {
    ...
    tailNoClear: registration === NA_TEXT ? "" : registration,
    registration: registration === NA_TEXT ? "" : registration,
    ...
    status: plane.flightState === 30 ? "途中" : firstMatchedValue(plane.groundTimeStr, "停场"),
...
function renderGroundPlaneRows(airport, planes) {
  return planes.slice(0, 18).map((plane) => {
    const registration = firstMatchedValue(plane.registration, plane.tailNoDisplay);
    const meta = [
      firstMatchedValue(plane.brandName),
      firstMatchedValue(plane.modelName),
      firstMatchedValue(plane.serviceProvider, plane.trusteeship)
    ].filter((value) => !missingValue(value)).join(" | ");
    const stateText = plane.flightState === 30 ? "途中" : firstMatchedValue(plane.groundTimeStr, serviceStatusLabel(plane.serviceStatus));
    return `
      <button type="button" class="related-flight ground-plane-row" data-tail="${escapeHtml(plane.tailNoEncrypted || "")}" data-registration="${escapeHtml(registration === NA_TEXT ? "" : registration)}">
        <span>
          <strong>${escapeHtml(registration)}</strong>
          <small>${escapeHtml(meta || displayOrDash(plane.modelName))}</small>
          <small>${escapeHtml(firstMatchedValue(plane.groundTimeStr, plane.trusteeship))}</small>
        </span>
        <span class="related-flight-meta">${escapeHtml(stateText)}</span>
      </button>`;
  }).join("");
}
...
function renderAirportTabList(airport) {
  ...
  if (tab === "ground") {
    ...
    if (planes.length) {
      list.innerHTML = renderGroundPlaneRows(airport, planes);
    ...
    if (state.detailLoads.has(`airport:${airport.airportCode || airport.iata}:ground`)) {
      list.innerHTML = `<p class="empty-related">停场飞机加载中...</p>`;
      return;
    }
    if (airport.airportGroundError) {
  ...
  list.innerHTML = related.map((jet) => `
    <button type="button" class="related-flight" data-id="${escapeHtml(jet.id)}">
      <span>
        <strong>${escapeHtml(displayOrDash(aircraftCallsignLabel(jet, firstMatchedValue(jet.registration, jet.tailNoClear))))}</strong>
...
function renderAirportDetailPanel(airport) {
  ...
  const info = mergePresentFields(detail.airportInfo, dynamic.airportInfo);
  ...
  setText("airportTrafficDate", firstMatchedValue(dynamic.date, detail.date));
  ...
  renderAirportNoticeList("airportApprovalRules", info.approvalRules, { fallbackTitle: "报批政策", limit: 2 });
  renderAirportNoticeList("airportWeatherNotices", detail.weatherNotices, { fallbackTitle: "天气预警", limit: 2 });
  syncSelectionDomState();
}
