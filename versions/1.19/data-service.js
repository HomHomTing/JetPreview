(function () {
  ...
  function historyTimeRef(item, fields, options = {}) {
    ...
    if (timeUtils?.makeFirstTimeRef) {
      return timeUtils.makeFirstTimeRef(candidates, options);
    }
    const matched = candidates.find((candidate) => candidate.value !== undefined && candidate.value !== null && candidate.value !== "");
    return makeTimeRef(matched?.value || "", {
      ...options,
      sourceField: matched?.sourceField || options.sourceField
    });
  }
