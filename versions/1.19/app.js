...
async function loadAircraftHistory(jet, options = {}) {
  ...
  const renderOptions = options.resetScroll
    ...
    : { preserveScroll: true };
  if (!dataService?.isEnabled() || !jet.tailNoEncrypted) {
  ...
  state.detailLoads.add(loadKey);
  try {
    const detail = await fetchAircraftHistoryDetail(jet.tailNoEncrypted);
    const currentJet = aircraftByDetailSeed(jet);
    if (!currentJet || !detail) {
      return;
    }
...
function setAircraftSegment(segment, options = {}) {
  ...
  if (nextSegment === "journey") {
    ...
    loadAircraftHistory(selectedAircraft(), { force: true, resetScroll: shouldResetHistoryScroll, resetToken });
...
function historyScrollResetShouldPin(token) {
  ...
  const expectedScrollTop = historyCurrentStatusScrollTop(scroller);
  const hasMovedAwayFromCurrentStatus = Math.abs(Number(scroller?.scrollTop || 0) - expectedScrollTop) > 4;
  if (!hardLocked && !timeline.applyingScrollRestore && hasMovedAwayFromCurrentStatus) {
...
function handleHistoryScrollIntent() {
  ...
  historyTimelineState().lastUserScrollAt = Date.now();
  nextHistoryScrollRestoreSeq();
...
function captureHistoryScrollState(options = {}, restoreSeq = currentHistoryScrollRestoreSeq()) {
  ...
  return {
    ...
    activeUserScroll: Date.now() - Number(timeline.lastUserScrollAt || 0) < 2500,
  ...
  window.requestAnimationFrame(() => {
    ...
    if (snapshot.forceTop) {
      if (historyScrollResetIsActive(snapshot.resetToken)) {
        restoreTop(historyCurrentStatusScrollTop(scroller));
...
function historyCurrentStatusScrollTop(scroller = historyDetailScroller()) {
  if (!scroller?.getBoundingClientRect) {
    return 0;
  }
  const panel = scroller.querySelector('.detail-segment-panel[data-aircraft-panel="journey"]:not([hidden])');
  const target = panel?.querySelector(".history-current-row") || panel?.querySelector(".history-current-block");
  ...
  const maxScrollTop = Math.max(0, scroller.scrollHeight - scroller.clientHeight);
  return Math.min(maxScrollTop, Math.max(0, scroller.scrollTop + targetRect.top - scrollerRect.top - 8));
}

function scrollHistoryToCurrentStatus(scroller = historyDetailScroller()) {
  if (!scroller) {
    return;
  }
  beginHistoryProgrammaticScroll();
  scroller.scrollTop = historyCurrentStatusScrollTop(scroller);
}
...
function scrollHistoryToListTop(scroller = historyDetailScroller()) {
  ...
  scrollHistoryToCurrentStatus(scroller);
}
...
function historyDateParts(epochMs, zone = "UTC") {
  ...
  try {
    ...
    return {
      ...
      monthNumber: new Date(Date.parse(`${parts.month} 1, ${parts.year}`)).getUTCMonth() + 1,
...
function historyMonthKey(item) {
  ...
  const parts = historyDateParts(epoch, departure?.displayZone);
  if (!parts) {
    return "unknown";
  }
  return `${parts.year}-${String(parts.monthNumber).padStart(2, "0")}`;
}
...
function historyAirportDisplay(item, side) {
  ...
  const name = matchedName === NA_TEXT ? "未知机场" : matchedName;
        ...
        ${buckets.map((bucket) => {
          ...
          return `
            <button type="button" class="${bucket.active ? "active" : ""} ${bucket.monthBoundary ? "is-month-boundary" : ""}"
              ${isYear ? `data-history-month="${escapeHtml(bucket.monthKey)}"` : `data-history-bucket="${escapeHtml(bucket.key)}"`}
...
function renderHistoryTimelineGroups(flights) {
  ...
  return `
    ...
    <div class="history-list-head" aria-hidden="true">
      <span>日期</span><span>航线</span><span>时长</span>
    </div>
...
function renderFlightHistoryTimeline(detail) {
  const filteredFlights = historyFlightsForTimeline(detail);
  const historyNow = historyTimelineNowEpoch(detail);
  const summary = historySummary(detail, filteredFlights);
  ...
  return `
    <div class="history-timeline-root ${historyTimelineState().overviewExpanded ? "overview-expanded" : ""} ${historyTimelineState().summaryCollapsed ? "summary-collapsed" : ""}">
      ...
      <div class="history-fixed-region">
        ${renderHistoryCurrentStatus(detail, filteredFlights, selectedAircraft(), historyNow)}
        ${renderHistoryControls(summary)}
        ${renderHistoryActivityBar(filteredFlights, historyNow)}
      </div>
      <div class="history-timeline-scroll" data-history-timeline-scroll>
        ${loading ? `<p class="empty-related">行程记录加载中...</p>` : renderHistoryTimelineGroups(filteredFlights)}
      </div>
    </div>
  `;
}
...
function handleHistoryTimelineScroll(event) {
  ...
  if (historyScrollResetIsActive()) {
    const expectedScrollTop = historyCurrentStatusScrollTop(scroller);
    const hasMovedAwayFromCurrentStatus = Math.abs(Number(scroller.scrollTop || 0) - expectedScrollTop) > 4;
    if (hasMovedAwayFromCurrentStatus) {
      const timeline = historyTimelineState();
      const hardLocked = Date.now() < Number(timeline.scrollResetHardUntil || 0);
