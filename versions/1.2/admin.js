...
const STORAGE_KEY = "aircraft-icon-control-console:v1.2:fr24-hd-png";
const CONSOLE_SCHEMA_VERSION = "1.2.0";
...
const configuredDefaultMappings = (iconSpecConfig.typeMappings || []).map((item) => ({
  ...
  updatedBy: item.updatedBy || "Seed 1.2 PDF spec"
}));
...
function createSeedState() {
  ...
  const snapshot = {
    snapshotId: "snap-20260731-001",
    version: iconSpecConfig.publishedVersion || "icon-map-1.2-fr24-hd",
    ...
    note: "1.2 mapping imported from the aircraft icon specification.",
    publishedBy: DEFAULT_USER,
  ...
  return {
    ...
    audit: [{
      ...
      target: iconSpecConfig.sourceTitle || "1.2 icon specification",
      user: DEFAULT_USER,
      time: "2026-07-31T10:00:00+08:00",
      detail: "Loaded local icon assets and type-code mappings."
    }]
  };
}
...
function resetLocalState() {
  if (!window.confirm("确认重置 1.2 后台本地草稿？已发布快照和审计也会恢复到初始状态。")) {
    return;
  }
