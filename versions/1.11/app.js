...
function explicitAircraftTypeCode(jet) {
  return firstAircraftTypeCodeFromSources(
    jet,
    jet.raw,
    jet.flightDetail?.planeInfo,
    jet.flightDetail?.raw,
    jet.flightDetail?.raw?.planeInfo,
    jet.planeDetail?.planeInfo,
    jet.planeDetail?.raw,
    jet.planeDetail?.raw?.planeInfo
  );
}
...
function aircraftTypeCodeForIcon(jet) {
  const explicitCode = explicitAircraftTypeCode(jet);
  const currentCode = normalizeAircraftTypeCode(jet.aircraftTypeCode);
  return explicitCode || (aircraftTypeCodeIsGeneric(currentCode) ? "" : currentCode);
}

function mappedAircraftIconKeyForJet(jet) {
  const typeCode = aircraftTypeCodeForIcon(jet);
  return aircraftIconKeyByTypeCode[typeCode] || "";
}

function resolvedAircraftIconKey(jet) {
  const mappedIconKey = mappedAircraftIconKeyForJet(jet);
  if (validAircraftIconKey(mappedIconKey)) {
    return mappedIconKey;
  }
  return defaultBusinessJetIconKey;
}
...
function applyAircraftTypeMetadata(jet) {
  ...
  const explicitCode = explicitAircraftTypeCode(jet);
  const currentCode = normalizeAircraftTypeCode(jet.aircraftTypeCode);
  const typeCode = explicitCode || (aircraftTypeCodeIsGeneric(currentCode) ? "" : currentCode);
  jet.manufacturer = type.manufacturer || jet.family || "Unknown";
  jet.icaoCode = explicitCode || jet.icaoCode || "";
  jet.aircraftTypeCode = typeCode;
  jet.fr24IconKey = resolvedAircraftIconKey(jet);
...
function iconTypeCodeProfileCandidate(jet) {
  ...
  if (!key
    ...
    || profileState.failedKeys.has(key)) {
    return false;
  }
  const typeCode = aircraftTypeCodeForIcon(jet);
  if (!aircraftTypeCodeIsGeneric(typeCode)) {
    return false;
  }
  const sizeText = [jet.planeSize, jet.model, jet.family, jet.sizeClass].filter(Boolean).join(" ").toLowerCase();
  const mayNeedSpecificUltraLongIcon = jet.sizeClass === "ultra-long"
    || sizeText.includes("超远程")
    || sizeText.includes("ultra");
  return mayNeedSpecificUltraLongIcon && aircraftIconKey(jet) === defaultBusinessJetIconKey;
}
...
function pumpIconTypeCodeProfileQueue() {
  ...
  while (profileState.loadingKeys.size < profileState.maxConcurrent && profileState.queue.length) {
    ...
    if (profileState.loadedKeys.has(next.key) || profileState.loadingKeys.has(next.key) || profileState.failedKeys.has(next.key)) {
      continue;
    }
...
async function loadIconTypeCodeProfile(task) {
  ...
  try {
    ...
    profileState.failedKeys.add(task.key);
  } finally {
...
function renderAircraftDetailPanel(jet) {
  ...
  setText("aircraftTypeBadge", firstMatchedValue(typeCode, jet.family, jet.planeSize));
