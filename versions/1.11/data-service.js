(function () {
  ...
  function adaptFlyingPlane(item, index, iconContext, sourcePid = "513008", requestParams = {}) {
    ...
    const callsignRegistration = displayRegistration(
      item.tailNoClear,
      item.registrationClear,
      item.tailNoDisplay,
      item.registration
    ...
    return {
      ...
      callsign: firstValue(item.callsign, item.flight, callsignRegistration, makeProtectedCallsign(uniqueKey)),
      registration,
      model: planeSize,
      category: sizeClass,
      sizeClass,
  ...
  function adaptFlightTrack(payload, iconContext) {
    ...
    return {
      ...
      serverNowEpochMs: normalizeEpochMs(base.serverNowEpochMs || base.currentTimeGmt8 || payload.serverNowEpochMs || payload.serverNow),
      ...
      updates: {
        ...
        positionTimestamp: lastPoint?.timestamp || normalizeEpochMs(base.serverNowEpochMs || base.currentTimeGmt8 || payload.serverNowEpochMs || payload.serverNow),
