(function () {
  ...
  class BizJetDataService {
    ...
    async getFlightHistory(tailNo, options = {}) {
      const requestOptions = normalizeFlightHistoryRequestOptions(options);
      const cacheKey = [
        tailNo,
        requestOptions.rangeDays || "",
        requestOptions.page || "",
        requestOptions.pageSize || "",
        requestOptions.status || "",
        requestOptions.airportCode || ""
      ].join(":");
      return this.cached("flightHistory", cacheKey, async () => {
        const payload = await this.request("513013", { tailNo, ...requestOptions });
  ...
  function normalizeFlightHistoryRequestOptions(options = {}) {
  ...
  function adaptRealtimeSnapshot(payload, iconContext, requestParams = {}, sourcePid = "513008") {
    ...
    const airports = Array.isArray(payload.airportList)
      ? payload.airportList.map((item) => adaptAirportListItem(item, loadedAt)).filter(Boolean)
      : [];
  ...
  function adaptAirportListItem(item, loadedAt = Date.now()) {
    ...
    const groundCount = normalizeGroundCount(item.groundNum);
    ...
    return {
      ...
      airportCode,
      name,
      nameCn: firstValue(item.airportName, item.airportFourName, ""),
      nameEn: firstValue(item.airportNameEn, ""),
      city,
      country: "",
      lat,
      lng,
      ...
      ground,
      groundCount,
      groundCountAvailable: groundCount !== null,
      groundCountSource: groundCount !== null ? "513008" : "",
      groundCountUpdatedAt: groundCount !== null ? loadedAt : null,
      delay: "-",
      weather: "-",
      level: displayLevel,
      displayLevel,
  ...
  function adaptFlyingPlane(item, index, iconContext, sourcePid = "513008", requestParams = {}) {
    ...
    return {
      ...
      fromName: firstValue(item.depAirportName, item.departureAirportName, item.fromName, ""),
      toName: firstValue(item.arrAirportName, item.arrivalAirportName, item.toName, ""),
      ...
      depart: firstValue(item.depart, item.depTime, item.departedTime, item.actualDepartureTime, "-"),
      arrive: firstValue(item.arrive, item.arrTime, item.arrivalTime, item.estimatedArrivalTime, "-"),
  ...
  function adaptFlightTrack(payload, iconContext) {
    ...
    const timeRefs = {
      ...
      actualArrival: makeTimeRef(firstValue(base.arrActualEpochMs, base.actualArrivalEpochMs, base.arrActualTime), {
        timeZone: arrZone,
        sourceField: "flightBaseInfo.arrActualEpochMs",
  ...
  function adaptAirportDetail(payload) {
    ...
    const groundCount = normalizeGroundCount(ground.groundNum);
    ...
    return {
      ...
      updates: {
        ...
        nameCn: firstValue(info.airportName, info.airportFourName, ""),
        nameEn: firstValue(info.airportNameEn, ""),
        city: firstValue(info.cityName, ""),
        ...
        departures,
        arrivals,
        ground: groundCount ?? 0,
        groundCount,
        groundCountAvailable: groundCount !== null,
        groundCountSource: groundCount !== null ? "513010" : "",
        groundCountUpdatedAt: groundCount !== null ? loadedAt : null,
  ...
  function adaptFlightHistory(payload) {
    const rows = firstArrayValue(
      ...
      payload?.rows,
      payload?.flightList,
      payload?.historyFlights,
      payload?.flights,
      payload
    );
    const flights = rows.map(adaptHistoryFlight).filter(Boolean);
    const serverNowEpochMs = normalizeEpochMs(firstValue(
      payload?.serverNowEpochMs,
      payload?.serverNow,
      payload?.serverTime,
      flights[0]?.serverNowEpochMs
    ));
    const currentPage = toInteger(payload?.currentPage ?? payload?.page ?? payload?.pageNum, 1);
    const nextPage = toInteger(payload?.nextPage, null);
    const hasNextRaw = payload?.hasNextPage ?? payload?.hasMore;
    const hasNextText = String(hasNextRaw || "").trim().toLowerCase();
    return {
      raw: payload,
      serverNowEpochMs,
      currentPage,
      hasNextPage: hasNextRaw === true || hasNextText === "true" || toInteger(hasNextRaw, 0) === 1 || (Number.isFinite(nextPage) && nextPage > currentPage),
      totalCount: toInteger(payload?.totalCount ?? payload?.total ?? payload?.totalRecords, flights.length),
      totalMinutes: toInteger(payload?.totalMinutes ?? payload?.totalDurationMinutes, null),
      monthlyStats: firstArrayValue(payload?.monthlyStats, payload?.monthStats, payload?.statistics?.monthlyStats),
      flights,
      groundAirportInfo: adaptHistoryGroundAirport(payload?.groundAirportInfo || payload?.groundAirport)
    };
  }

  function historyTimeRef(item, fields, options = {}) {
    const candidates = fields.map((field) => ({
      sourceField: `513013.${field}`,
      value: item[field]
    })).filter((candidate) => candidate.value !== undefined && candidate.value !== null && candidate.value !== "");
    const refs = candidates.map((candidate) => makeTimeRef(candidate.value, {
      ...options,
      sourceField: candidate.sourceField || options.sourceField
    }));
    return refs.find((ref) => ref.epochMs !== null) || refs[0] || makeTimeRef("", options);
  }
  ...
  function adaptHistoryFlight(item) {
    ...
    const times = {
      scheduledDeparture: historyTimeRef(item, ["depScheduledEpochMs", "scheduledDepartureEpochMs", "stdEpochMs", "stdTime", "depPlanTime"], {
        timeZone: depZone,
        semantic: "history_scheduled_departure"
      }),
      actualDeparture: historyTimeRef(item, ["depActualEpochMs", "actualDepartureEpochMs", "atdEpochMs", "depTime1", "depActualTime"], {
        timeZone: depZone,
        semantic: "history_actual_departure"
      }),
      scheduledArrival: historyTimeRef(item, ["arrScheduledEpochMs", "scheduledArrivalEpochMs", "staEpochMs", "staTime", "arrPlanTime"], {
        timeZone: arrZone,
        semantic: "history_scheduled_arrival"
      }),
      actualArrival: historyTimeRef(item, ["arrActualEpochMs", "actualArrivalEpochMs", "ataEpochMs", "arrTime1", "arrActualTime"], {
        timeZone: arrZone,
        semantic: "history_actual_arrival"
      }),
      estimatedArrival: historyTimeRef(item, ["arrEstimatedEpochMs", "estimatedArrivalEpochMs", "etaEpochMs", "etaTime", "arrEstimateTime"], {
        timeZone: arrZone,
        semantic: "history_estimated_arrival"
      })
    };
    const depTimeRef = times.actualDeparture.epochMs !== null ? times.actualDeparture : times.scheduledDeparture;
    const arrTimeRef = times.actualArrival.epochMs !== null
      ? times.actualArrival
      : times.estimatedArrival.epochMs !== null
    ...
    return {
      ...
      depIata: cleanCode(item.depIata || item.depIataCode || item.depAirportIata || item.depAirportCode),
      depIcao: cleanCode(item.depIcao || item.depIcaoCode || item.depAirportIcao || item.depAirportIcaoCode),
      ...
      arrIata: cleanCode(item.arrIata || item.arrIataCode || item.arrAirportIata || item.arrAirportCode),
      arrIcao: cleanCode(item.arrIcao || item.arrIcaoCode || item.arrAirportIcao || item.arrAirportIcaoCode),
      ...
      durationMinutes: toInteger(item.durationMinutes ?? item.flightDurationMinutes, null),
      delayMinutes: toInteger(item.delayMinutes, null)
    };
  }
  ...
  function adaptAirportGround(payload) {
    ...
    const groundModels = Array.isArray(ground.groundModels)
      ...
      : [];
    const explicitGroundCount = normalizeGroundCount(ground.groundNum);
    const groundCount = explicitGroundCount ?? groundPlanes.length;
    const airportCode = cleanCode(info.airportCode);
    const icaoCode = cleanCode(info.icaoCode);
    return {
      raw: payload,
      ...
      updates: {
        ...
        ground: groundCount,
        groundCount,
        groundCountAvailable: true,
        groundCountSource: "513014",
        groundCountUpdatedAt: loadedAt,
        timeZone: info.timeZone,
        nameCn: firstValue(info.airportName, info.airportFourName, ""),
        nameEn: firstValue(info.airportNameEn, ""),
        city: firstValue(info.cityName, ""),
  ...
  function adaptGroundPlane(item) {
    ...
    const registrationClear = clearDisplayRegistration(
      item.tailNoClear,
      item.registrationClear,
      item.registration
    );
    return {
      ...
      registrationClear,
      tailNoClear: registrationClear,
      registration: displayRegistration(registrationClear, item.tailNoDisplay),
      ...
      flightState: toInteger(item.flightState, null),
      airportCode: cleanCode(item.airportCode),
      depAirport: cleanCode(item.depAirport || item.depAirportCode),
      arrAirport: cleanCode(item.arrAirport || item.arrAirportCode || item.airportCode),
      arrDate: firstValue(item.arrDate, item.arrivalDate, ""),
      arrTime: firstValue(item.arrTime, item.arrivalTime, ""),
      extraInfo: firstValue(item.extraInfo, ""),
      traceId: firstValue(item.traceId, item.id, ""),
  ...
  function clearDisplayRegistration(...values) {
    const registration = displayRegistration(...values);
    return /[*＊•●·]{2,}|X{3,}/i.test(registration) ? "" : registration;
  }
  ...
  function normalizeGroundCount(value) {
    if (value === null || value === undefined || value === "") {
      return null;
    }
    const number = Number.parseInt(value, 10);
