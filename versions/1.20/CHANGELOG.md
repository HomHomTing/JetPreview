...
- Frozen under `versions/1.20/` and published as the current online/GitHub release target.
- Rebuilds the selected-airport panel around four operational tabs: ground fleet, movement timeline, operations analysis, and airport information.
- Shows complete Chinese/English airport names, IATA/ICAO codes, city, and UTC-formatted airport timezone data.
- Expands the ground fleet from truncated samples to all returned parked and inbound aircraft, with plaintext registrations, parking-duration labels, search/sort controls, and model/brand composition filters.
- Builds the airport movement board from current airport and realtime movement data, then enriches missing route origins from 513013 aircraft history records with bounded concurrency and caching.
- Presents paired local departure and arrival times beneath full origin/destination airport names, codes, complete aircraft model, direction, and movement status.
- Adds parking-aircraft count badges to airport pins using 513008 as the map source and selected-airport detail responses as local calibration sources.
- Couples each parking badge to the airport icon visual container and render queue so icon and badge always appear, transform, and disappear together across zoom thresholds.
- Removes persistent airport text labels at every scale while preserving complete hover and selected-airport information.
- Keeps aircraft markers above airports and preserves selected/hovered/route endpoint airports through scale filtering.
