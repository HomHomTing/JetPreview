(function () {
  const sourceDate = "2026-07-31T10:00:00+08:00";
  ...
  const iconImagePaths = Object.fromEntries(
    groups.map((group) => [group.iconKey, `assets/aircraft-icons/fr24-hd/${group.iconKey}.png`])
  );
  ...
  const compatibilityMappings = [
    {
      ...
      effectiveFrom: "",
      effectiveTo: "",
      notes: "Compatibility: existing 1.2 sample business jet code, rendered by the source rule that all business jets use LJ60.",
      updatedAt: sourceDate,
      updatedBy: "Seed 1.2 PDF spec"
    }
  ];
  ...
  const iconAssets = groups.map((group) => ({
    ...
    sourceMode: "document-extracted-fr24-hd-png",
    ...
    status: "Active",
    licenseRef: "user-provided-reference",
    checksum: `fr24-hd-${group.iconKey}-20260803`,
    updatedAt: sourceDate,
    updatedBy: "Seed 1.2 PDF spec"
  }));
  ...
  root.AIRCRAFT_ICON_CONFIG = {
    schemaVersion: "1.2.0",
    sourceVersion: "claude-artifact-6971bd08-20260731",
    publishedVersion: "icon-map-1.2-fr24-hd",
