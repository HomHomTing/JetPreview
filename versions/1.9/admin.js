...
const DEFAULT_ICON_KEY = iconSpecConfig.defaultIconKey || "lj45";
const STORAGE_KEY = "aircraft-icon-control-console:v1.7:template-shadow-fr24yellow";
const CONSOLE_SCHEMA_VERSION = "1.7.0";
...
const configuredDefaultMappings = (iconSpecConfig.typeMappings || []).map((item) => ({
  aircraftTypeCode: String(item.aircraftTypeCode || "").trim(),
  manufacturer: String(item.manufacturer || "Multiple").trim(),
  modelNames: Array.isArray(item.modelNames) && item.modelNames.length ? item.modelNames : [item.aircraftTypeCode],
...
const defaultMappings = configuredDefaultMappings.length ? configuredDefaultMappings : [
  mapping("GLF6", "Gulfstream", ["Gulfstream G650ER"], "Business Jet", "ultra-long", "lj45", "Business jet baseline mapping."),
  mapping("GL7T", "Bombardier", ["Bombardier Global 7500"], "Business Jet", "ultra-long", "lj45", "Business jet baseline mapping."),
  mapping("FA8X", "Dassault", ["Dassault Falcon 8X"], "Business Jet", "long-range", "lj45", "Business jet baseline mapping."),
  mapping("C700", "Cessna", ["Cessna Citation Longitude"], "Business Jet", "midsize", "lj45", "Business jet baseline mapping."),
  mapping("GLF5", "Gulfstream", ["Gulfstream G550"], "Business Jet", "long-range", "lj45", "Business jet baseline mapping."),
  mapping("GLEX", "Bombardier", ["Bombardier Global 6000"], "Business Jet", "long-range", "lj45", "Business jet baseline mapping."),
  mapping("E550", "Embraer", ["Embraer Praetor 600"], "Business Jet", "super-midsize", "lj45", "Business jet baseline mapping."),
  mapping("PC24", "Pilatus", ["Pilatus PC-24"], "Business Jet", "light", "lj45", "Business jet baseline mapping."),
  mapping("CL35", "Bombardier", ["Bombardier Challenger 350"], "Business Jet", "super-midsize", "lj45", "Business jet baseline mapping."),
  mapping("FA7X", "Dassault", ["Dassault Falcon 7X"], "Business Jet", "long-range", "lj45", "Business jet baseline mapping."),
  mapping("GA5C", "Gulfstream", ["Gulfstream G500"], "Business Jet", "long-range", "lj45", "Business jet baseline mapping."),
...
function handleChange(event) {
  ...
  if (target.id === "selectAllMappings") {
    state.selectedCodes = target.checked ? getFilteredMappings().map((item) => item.aircraftTypeCode) : [];
    renderTypeMappings();
    saveState();
    return;
  }
          ...
          ${topIcons.map((item) => `
            <div class="rule-row" data-icon-card="${escapeHtml(item.icon.iconKey)}">
              <div class="mini-aircraft-cell">
                ...
                <span><strong>${escapeHtml(item.icon.displayName)}</strong><span>${escapeHtml(item.icon.iconKey)} · ${item.count} 个 type code</span></span>
              </div>
...
function renderTypeMappings() {
  ...
  const allFilteredSelected = mappings.length > 0 && mappings.every((item) => state.selectedCodes.includes(item.aircraftTypeCode));
  ...
  document.getElementById("mappingsView").innerHTML = `
    <div class="toolbar">
      <div class="field-cluster">
        <label class="search-field">
          <svg><use href="#icon-search"></use></svg>
          <input id="mappingSearch" type="search" value="${escapeHtml(state.filters.query)}" placeholder="搜索 type code / 制造商 / 机型 / icon key">
        </label>
...
function renderPreviewLab() {
  ...
  const selectedCode = selectedMapping?.aircraftTypeCode || "";
  ...
  document.getElementById("previewView").innerHTML = `
    <div class="preview-controls">
      <div class="form-field">
        <label for="previewTypeSelect">Aircraft Type Code</label>
        ${selectHtml("previewTypeSelect", state.mappings.map((item) => item.aircraftTypeCode), selectedCode, {}, "")}
      </div>
      <div class="form-field">
  ...
  document.getElementById("publishView").innerHTML = `
    <div class="section-grid">
      <section class="panel">
        ...
        <div class="diff-grid">
          ${diffBox("新增", diff.added.length, "new type codes")}
      ...
      <div class="section-grid section-grid--two">
        ...
        <section class="panel">
          <div class="panel-header">
            <div>
              <h2>发布操作</h2>
              <p>发布会生成新的本地快照，供后续展示端接入。</p>
            </div>
          </div>
          <div class="form-grid">
            <div class="form-field">
              <label for="releaseNote">发布说明 <small>必填</small></label>
              <textarea id="releaseNote" placeholder="例如：调整 business jet icon key 映射，补充 A320 / ATR / H135 示例。">${escapeHtml(state.releaseNote)}</textarea>
            </div>
            <div class="button-row">
              <button class="button button--primary" type="button" data-action="publish-draft"><svg><use href="#icon-publish"></use></svg><span>发布草稿</span></button>
      ...
      <div class="section-grid section-grid--two">
        <section class="panel">
          ...
          <div class="import-box">
            <textarea id="importText" spellcheck="false" placeholder="JSON: { &quot;typeMappings&quot;: [...] }&#10;CSV: aircraftTypeCode,manufacturer,modelNames,aircraftCategory,sizeClass,iconKey,notes"></textarea>
...
function renderSettings() {
  const schemaSample = {
    ...
    typeMappings: "AircraftTypeMapping[]",
  ...
  document.getElementById("settingsView").innerHTML = `
    <div class="section-grid section-grid--two">
      <section class="panel">
        ...
        <div class="rule-list">
          ${ruleRow("Type code", "2-10 位大写字母或数字，保存时自动转大写。")}
          ${ruleRow("唯一性", "同一环境内 Active type code 只能对应一个 icon key。")}
          ${ruleRow("图标引用", "Icon key 必须存在且为 Active；归档图标不能用于新映射。")}
          ${ruleRow("必填信息", "制造商、机型名称、分类、尺寸、icon key 均不能为空。")}
          ${ruleRow("发布说明", "发布草稿前必须填写说明，便于回滚和审计。")}
        </div>
      </section>
...
function renderEditor() {
  ...
  const originalCode = state.editorDraft ? "" : selected.aircraftTypeCode;
  ...
  document.getElementById("editorPanel").innerHTML = `
    ...
    <div class="editor-preview">
      ...
      <div>
        <strong id="editorPreviewTitle">${escapeHtml(selected.aircraftTypeCode || "NEW")}</strong>
    ...
    <form id="mappingForm" data-original-code="${escapeHtml(originalCode)}">
      <div class="form-grid">
        <div class="form-field">
          <label for="aircraftTypeCode">Aircraft type code <small>必填</small></label>
          <input id="aircraftTypeCode" name="aircraftTypeCode" value="${escapeHtml(selected.aircraftTypeCode)}" maxlength="10" autocomplete="off" placeholder="GLF6">
        </div>
        <div class="form-field">
...
function iconCard(icon) {
  const usage = state.mappings
    ...
    .map((item) => item.aircraftTypeCode);
  ...
  return `
    <div class="icon-card" data-icon-card="${escapeHtml(icon.iconKey)}">
      ...
      <div class="icon-card__meta">
        ...
        <small>${usage.length ? escapeHtml(usage.join(", ")) : "未被引用"}</small>
      </div>
    </div>
  `;
}
...
function mappingRow(item) {
  const isSelected = state.selectedCode === item.aircraftTypeCode;
  const isChecked = state.selectedCodes.includes(item.aircraftTypeCode);
  const draftState = draftLabel(item);
  const icon = getIcon(item.fr24IconKey);

  return `
    <tr data-code-row="${escapeHtml(item.aircraftTypeCode)}" class="${isSelected ? "is-selected" : ""}">
      <td><input type="checkbox" data-select-code value="${escapeHtml(item.aircraftTypeCode)}" ${isChecked ? "checked" : ""} aria-label="选择 ${escapeHtml(item.aircraftTypeCode)}"></td>
      <td><span class="code-chip">${escapeHtml(item.aircraftTypeCode)}</span></td>
  ...
  return `
    <div class="table-wrap">
      <table class="data-table">
        ...
        <tbody>
          ${rows.map(({ type, item }) => `
            <tr data-code-row="${escapeHtml(item.aircraftTypeCode)}">
...
function getMapping(code) {
  return state.mappings.find((item) => item.aircraftTypeCode === code);
}
...
function duplicateSelectedMapping() {
  ...
  state.editorDraft = {
    ...
    status: "Draft",
    notes: `${selected.notes || ""} Copied from ${selected.aircraftTypeCode}.`.trim(),
  ...
  render();
  focusEditorCode();
  showToast("已复制为新草稿，请填写新的 type code。");
}
...
function archiveSelectedMapping() {
  ...
  addAudit("Archive mapping", selected.aircraftTypeCode, `${selected.aircraftTypeCode} archived.`);
  render();
  showToast(`${selected.aircraftTypeCode} 已归档为草稿变更。`);
}
...
function deleteSelectedMapping() {
  ...
  const published = state.publishedMappings.some((item) => item.aircraftTypeCode === selected.aircraftTypeCode);
  if (published) {
    showToast("已发布过的映射请使用归档，避免历史快照断链。");
    return;
  }
  if (!window.confirm(`确认移除 ${selected.aircraftTypeCode} 这条未发布草稿？`)) {
    return;
  }
  state.mappings = state.mappings.filter((item) => item.aircraftTypeCode !== selected.aircraftTypeCode);
  state.selectedCode = state.mappings[0]?.aircraftTypeCode || "";
  addAudit("Delete draft", selected.aircraftTypeCode, "Removed unpublished draft mapping.");
  render();
}

function saveMappingForm(form) {
  const originalCode = form.dataset.originalCode;
  const draft = readMappingForm(form);
  ...
  const existingIndex = state.mappings.findIndex((item) => item.aircraftTypeCode === draft.aircraftTypeCode);
  const originalIndex = originalCode ? state.mappings.findIndex((item) => item.aircraftTypeCode === originalCode) : -1;
  const next = {
    ...draft,
    updatedAt: new Date().toISOString(),
    updatedBy: DEFAULT_USER
  ...
  if (existingIndex >= 0 && state.mappings[existingIndex].aircraftTypeCode !== originalCode) {
    showToast(`${draft.aircraftTypeCode} 已存在，不能重复保存。`);
    return;
  }
  ...
  if (originalCode && originalCode !== next.aircraftTypeCode) {
    state.mappings = state.mappings.filter((item, index) => item.aircraftTypeCode !== originalCode || index === state.mappings.findIndex((row) => row.aircraftTypeCode === next.aircraftTypeCode));
  }

  state.selectedCode = next.aircraftTypeCode;
  state.preview.selectedTypeCode = next.aircraftTypeCode;
  state.preview.selectedIconKey = next.fr24IconKey;
  state.editorDraft = null;
  addAudit("Save draft", next.aircraftTypeCode, `${next.aircraftTypeCode} -> ${next.fr24IconKey}`);
  render();
  showToast(`${next.aircraftTypeCode} 已保存为草稿。`);
}

function readMappingForm(form) {
  const data = new FormData(form);
  return {
    aircraftTypeCode: normalizeTypeCode(data.get("aircraftTypeCode")),
...
function updateEditorLivePreview() {
  ...
  title.textContent = draft.aircraftTypeCode || "NEW";
...
function applyBulkIcon() {
  if (!state.selectedCodes.length) {
    showToast("请先勾选要批量处理的 type code。");
    return;
  }
  ...
  state.mappings = state.mappings.map((item) => state.selectedCodes.includes(item.aircraftTypeCode)
...
function importDraft() {
  ...
  try {
    const imported = parseImportText(text).map(normalizeImportedMapping).filter(Boolean);
    let added = 0;
    let updated = 0;
    imported.forEach((item) => {
      const index = state.mappings.findIndex((mappingItem) => mappingItem.aircraftTypeCode === item.aircraftTypeCode);
      if (index >= 0) {
...
function exportCsv() {
  const headers = ["aircraftTypeCode", "manufacturer", "modelNames", "aircraftCategory", "sizeClass", "iconKey", "status", "notes"];
  const rows = state.mappings.map((item) => headers.map((key) => {
    const value = key === "modelNames" ? (item.modelNames || []).join("; ") : key === "iconKey" ? item.fr24IconKey : item[key];
...
function publishDraft() {
  const issues = validateMappings();
  const errors = issues.filter((issue) => issue.severity === "error");
  const releaseNote = String(document.getElementById("releaseNote")?.value || state.releaseNote || "").trim();
  if (errors.length) {
  ...
  state.publishedMappings = deepClone(state.mappings);
  state.snapshots.unshift(snapshot);
  state.releaseNote = "";
  addAudit("Publish snapshot", version, releaseNote);
  render();
  showToast(`${version} 已发布。`);
}
...
function rollbackToSnapshot(snapshotId) {
  ...
  state.mappings = deepClone(snapshot.mappings);
  const diff = getMappingDiff();
...
function createSnapshot(version, note, rollbackOf, diff = getMappingDiff()) {
  return {
    ...
    rollbackOf,
    mappings: deepClone(state.mappings)
  };
}
...
function validateMappings() {
  ...
  state.mappings.forEach((item) => {
    const rowIssues = validateMappingDraft(item, item.aircraftTypeCode, true);
    rowIssues.forEach((issue) => issues.push({ ...issue, message: `${item.aircraftTypeCode || "NEW"}: ${issue.message}` }));

    if (item.status !== "Archived") {
      if (activeCodes.has(item.aircraftTypeCode)) {
        issues.push({ severity: "error", message: `${item.aircraftTypeCode}: Active type code 重复。` });
      }
      activeCodes.set(item.aircraftTypeCode, true);
    }
  });
  return issues;
}
...
function validateMappingDraft(item, originalCode = "", skipDuplicate = false) {
  const issues = [];
  const code = normalizeTypeCode(item.aircraftTypeCode);
  const icon = getIcon(item.fr24IconKey);

  if (!code) {
    issues.push({ severity: "error", message: "Aircraft type code 必填。" });
  } else if (!/^[A-Z0-9]{2,10}$/.test(code)) {
    issues.push({ severity: "error", message: "Aircraft type code 需要 2-10 位大写字母或数字。" });
  }

  if (!skipDuplicate && code) {
    const duplicate = state.mappings.find((mappingItem) => mappingItem.aircraftTypeCode === code && mappingItem.aircraftTypeCode !== originalCode);
    if (duplicate) {
...
function getMappingDiff() {
  const published = new Map(state.publishedMappings.map((item) => [item.aircraftTypeCode, item]));
  const current = new Map(state.mappings.map((item) => [item.aircraftTypeCode, item]));
  const added = [];
  const changed = [];
  const archived = [];
  const removed = [];
  ...
  state.mappings.forEach((item) => {
    const base = published.get(item.aircraftTypeCode);
    if (!base) {
      added.push(item);
      return;
    }
  ...
  state.publishedMappings.forEach((item) => {
    if (!current.has(item.aircraftTypeCode)) {
      removed.push(item);
    }
  });
...
function draftLabel(item) {
  const base = state.publishedMappings.find((publishedItem) => publishedItem.aircraftTypeCode === item.aircraftTypeCode);
  if (!base) {
...
function parseImportText(text) {
  ...
  if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
    ...
    throw new Error("JSON 中没有找到 typeMappings。");
  }
...
function normalizeImportedMapping(row) {
  const code = normalizeTypeCode(row.aircraftTypeCode || row.typeCode || row.code);
  if (!code) {
    return null;
  }
  return {
...
function createSeedState() {
  const mappings = deepClone(defaultMappings);
  const snapshot = {
    snapshotId: "snap-20260803-icon-template-shadow-fr24yellow",
    version: iconSpecConfig.publishedVersion || "icon-map-1.7-template-shadow-fr24yellow",
    ...
    note: "1.7 mapping synchronized with the template shadow FR24-yellow 1024px icon set.",
    publishedBy: DEFAULT_USER,
    publishedAt: iconSpecConfig.sourceDate || "2026-08-03T20:00:00+08:00",
    rollbackOf: null,
  ...
  return {
    ...
    audit: [{
      ...
      target: iconSpecConfig.publishedVersion || "icon-map-1.7-template-shadow-fr24yellow",
      user: DEFAULT_USER,
      time: iconSpecConfig.sourceDate || "2026-08-03T20:00:00+08:00",
      detail: "Loaded template shadow FR24-yellow icon assets and type-code mappings."
    }]
  };
}
...
function loadState() {
  try {
    ...
    return {
      ...
      iconFilters: { ...seed.iconFilters, ...(parsed.iconFilters || {}) },
      preview: { ...seed.preview, ...(parsed.preview || {}) },
      icons: Array.isArray(parsed.icons) ? parsed.icons : seed.icons,
      mappings: Array.isArray(parsed.mappings) ? parsed.mappings : seed.mappings,
      publishedMappings: Array.isArray(parsed.publishedMappings) ? parsed.publishedMappings : seed.publishedMappings,
      snapshots: Array.isArray(parsed.snapshots) ? parsed.snapshots : seed.snapshots,
      audit: Array.isArray(parsed.audit) ? parsed.audit : seed.audit
    };
  } catch (error) {
...
function resetLocalState() {
  if (!window.confirm("确认重置 1.7 后台本地草稿？已发布快照和审计也会恢复到当前图标版本初始状态。")) {
    return;
  }
...
function publicMapping(item) {
  ...
  return { ...rest, iconKey: fr24IconKey };
}
