...
function renderAircraftDetailPanel(jet) {
  ...
  const serverNow = routeTimes.serverNow.epochMs || parsePanelEpoch(jet.updatedAtEpochMs) || Date.now();
  ...
  setText("scheduledDeparture", formatRouteTime(routeTimes.scheduledDeparture));
  setText("departedTime", formatRouteTime(routeTimes.actualDeparture));
  setText("scheduledArrival", maybeAppendAcrossDays(formatRouteTime(routeTimes.scheduledArrival), base.acrossDays));
  setText("arrivalTime", maybeAppendAcrossDays(formatRouteTime(routeTimes.estimatedArrival), base.acrossDays));
