# Global BizJet Ops 1.7

V1.7 prototype for a web-based aircraft operations map, icon test system, aircraft type-code icon control console, private operations database integration, normalized selected-aircraft route visualization, and physically directed low-altitude aircraft ground projections.
...
Version 1.7 is the active iteration in the project root.
...
Version 1.7 implements `docs/aircraft-ground-shadow-projection-requirements-v1.7.md`: aircraft-local ellipses and colored glow filters are removed; aircraft shadows use a dedicated non-interactive map layer, low-altitude AGL/solar projection, an all-altitude visual fallback, geographic shadow destinations, density limits, and a persisted layer toggle.
...
Aircraft track refresh and continuity requirements for version 1.6 are documented in `docs/aircraft-track-refresh-continuity-requirements-v1.6.md`.
...
- Version 1.7 keeps the home map in live-only mode: aircraft, airport, route, and detail panels render data from the configured private database only.
- Version 1.7 continues using the provided `513008` snapshot endpoint for aircraft and airport loading. Optional viewport parameters are sent to `513008`; no extra pid request is made.
