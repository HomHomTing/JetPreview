(function () {
  ...
  function adaptFlyingPlane(item, index, iconContext, sourcePid = "513008", requestParams = {}) {
    ...
    return {
      ...
      registration: displayRegistration(item.tailNoDisplay, item.registration, "Protected"),
      model: planeSize,
      category: sizeClass,
      sizeClass,
      family: planeSize,
  ...
  function adaptViewportAircraft(item, index, iconContext, sourcePid = "513008", requestParams = {}) {
    ...
    return {
      ...
      registration: displayRegistration(item.tailNoDisplay, item.registrationDisplay, item.registration, item.tailNo, "Protected"),
      model,
  ...
  function adaptFlightTrack(payload, iconContext) {
    ...
    return {
      ...
      serverNowEpochMs: normalizeEpochMs(payload.serverNowEpochMs || payload.serverNow),
      ...
      updates: {
        ...
        registration: displayRegistration(plane.tailNoDisplay, plane.registration, plane.tailNo, "Protected"),
        ...
        positionTimestamp: lastPoint?.timestamp || normalizeEpochMs(payload.serverNowEpochMs || payload.serverNow),
  ...
  function displayRegistration(...values) {
    ...
    if (!value || encryptedTailNoLike(value)) {
      return "";
    }
    return value;
  }
