...
const aircraftGroundProjectionConfig = {
  ...
  visualMinOffsetPx: appConfig.performance?.groundProjection?.visualMinOffsetPx ?? 9,
  visualMaxOffsetPx: appConfig.performance?.groundProjection?.visualMaxOffsetPx ?? 18,
...
function trackStyleForZoom({ zoom, selected, routeFocused, stale }) {
  ...
  return {
    ...
    haloWidth: selected ? interpolateZoomWidth(haloStops, zoom) : 0,
    ...
    haloOpacity: (routeFocused ? routeStyle.focusHaloOpacity : routeStyle.selectedHaloOpacity) * opacityFactor,
...
const state = {
  labels: true,
  trails: true,
  groundProjections: readBooleanPreference(aircraftGroundProjectionConfig.preferenceKey, true),
  airports: true,
...
class LeafletMapEngine {
  constructor() {
    this.type = "leaflet";
    this.groundProjectionMarkers = new Map();
    ...
    this.map.createPane("aircraftGroundProjections");
    this.map.getPane("aircraftGroundProjections").style.zIndex = 390;
    this.map.getPane("aircraftGroundProjections").style.pointerEvents = "none";
  ...
  renderGroundProjections(projections) {
    const activeIds = new Set();
    projections.forEach((projection) => {
      activeIds.add(projection.id);
      const icon = L.divIcon({
        className: "aircraft-ground-projection-leaflet-icon",
        html: groundProjectionElementHtml(projection),
        iconSize: [64, 64],
        iconAnchor: [32, 32]
      });
      if (!this.groundProjectionMarkers.has(projection.id)) {
        this.groundProjectionMarkers.set(projection.id, L.marker(
          [projection.projectionLat, projection.projectionLng],
          {
            pane: "aircraftGroundProjections",
            interactive: false,
            keyboard: false,
            icon
          }
      ...
      const marker = this.groundProjectionMarkers.get(projection.id);
      marker.setLatLng([projection.projectionLat, projection.projectionLng]);
      marker.setIcon(icon);
      const element = marker.getElement()?.querySelector(".aircraft-ground-projection");
      if (element) {
        applyGroundProjectionElementStyle(element, projection);
      }
    });
    this.groundProjectionMarkers.forEach((marker, id) => {
      if (!activeIds.has(id)) {
        this.map.removeLayer(marker);
        this.groundProjectionMarkers.delete(id);
      }
    });
  }
  ...
  clearTrackRecord(record) {
    ...
    const layers = record.cores instanceof Map
      ? [...record.cores.values(), ...(record.halos || [])]
  ...
  setTrack(id, points, selected, options = {}) {
    ...
    const segments = trackSegments(points, selected);
    const record = state.tracks.get(id) || { cores: new Map(), halos: [] };
    if (!(record.cores instanceof Map)) {
      this.clearTrackRecord(record);
      record.cores = new Map();
      record.halos = [];
    }
    const haloPaths = selected ? actualTrackPaths(segments) : [];
...
class GoogleMapEngine {
  constructor() {
    ...
    this.groundProjectionOverlay = createGoogleGroundProjectionOverlay(this.map);
  ...
  renderGroundProjections(projections) {
    this.groundProjectionOverlay.setProjections(projections);
  }
  ...
  setTrack(id, points, selected, options = {}) {
    ...
    const segments = trackSegments(points, selected);
    const record = this.lines.get(id) || { cores: new Map(), halos: [] };
    if (!(record.cores instanceof Map)) {
      this.clearTrackRecord(record);
      record.cores = new Map();
      record.halos = [];
...
function groundProjectionForAircraft(jet) {
  ...
  const physicalProjection = canUsePhysicalProjection
  ...
  const visualStyle = projectionMode === "physical"
    ? {
      sizeRatio: 0.86,
      opacity: (0.38 + (0.18 - 0.38) * heightRatio) * (clamped ? 0.75 : 1),
      blurPx: 0.5 + (2 - 0.5) * heightRatio,
      scaleX: 0.92 + (0.78 - 0.92) * heightRatio,
      scaleY: 0.76 + (0.62 - 0.76) * heightRatio
    }
    : {
      sizeRatio: 0.82,
      opacity: 0.36 + (0.25 - 0.36) * heightRatio,
      blurPx: 0.6 + (1.4 - 0.6) * heightRatio,
      scaleX: 0.92 + (0.84 - 0.92) * heightRatio,
      scaleY: 0.76 + (0.66 - 0.76) * heightRatio
    };
  return {
    id: jet.id,
    visible: true,
...
function trackBreakReason(start, end) {
  ...
  if (elapsedMs > routeStyle.maxGapMs) {
    return "coverage_gap";
  }
  const impliedSpeedKt = greatCircleDistanceNm(start, end) * 3600000 / elapsedMs;
  return impliedSpeedKt > routeStyle.maxImpliedSpeedKt ? "speed_anomaly" : "";
}
...
function renderAircraft() {
  ...
  state.map.renderGroundProjections?.(groundProjectionsForAircraft(aircraftMarkers));
  ...
  [...aircraftMarkers].sort((a, b) => {
    if (aircraftIsSelected(a) && !aircraftIsSelected(b)) return 1;
    if (!aircraftIsSelected(a) && aircraftIsSelected(b)) return -1;
    return 0;
  }).forEach((jet) => {
    const selected = aircraftIsSelected(jet);
...
function selectAircraft(id, shouldPan = true) {
  ...
  const from = airportByCode(jet.from);
  const to = airportByCode(jet.to);
  const position = currentPosition(jet);
  const heading = aircraftHeading(jet);
  state.selectedKind = "aircraft";
  state.selectedId = id;
...
function bindEvents() {
  ...
  const groundProjectionToggle = document.getElementById("groundProjectionToggle");
  airportLayerSelect.value = state.airportLayerMode;
  groundProjectionToggle.checked = state.groundProjections;
  ...
  groundProjectionToggle.addEventListener("change", (event) => {
    state.groundProjections = event.target.checked;
    writeBooleanPreference(aircraftGroundProjectionConfig.preferenceKey, state.groundProjections);
    renderAircraft();
  });
