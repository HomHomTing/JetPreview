...
FR24 实测 selected route 中未将未来计划航线绘制为同等高度色阶线。因此本系统 1.6 默认：

- 不绘制未来计划航线。
- 如果未来接入飞行计划数据，可作为独立 planned route layer。
- planned route 必须使用浅灰/浅黄长虚线，不能使用高度色阶，避免用户误认为已飞轨迹。
- planned route 默认关闭，仅在用户主动打开计划航线分析时显示。
...
- 相邻点时间差超过 `180s`，默认切换为估算段或断开。
- 相邻点速度异常时，不直接连线，需要标记 weak/estimated。
