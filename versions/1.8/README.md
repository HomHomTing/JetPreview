# Global BizJet Ops 1.8

V1.8 prototype for a web-based aircraft operations map, icon test system, aircraft type-code icon control console, private operations database integration, normalized selected-aircraft route visualization, selected-track live continuity, and selected-flight planned destination links.
...
Version 1.8 is frozen in this directory. The project root has moved on to the next active iteration.
...
Airport selected-panel requirements for the next airport-detail iteration are documented in `docs/airport-selected-panel-requirements-v1.9.md`.
...
- Version 1.8 keeps the home map in live-only mode: aircraft, airport, route, and detail panels render data from the configured private database only.
- Version 1.8 continues using the provided `513008` snapshot endpoint for aircraft and airport loading. Optional viewport parameters are sent to `513008`; no extra pid request is made.
...
- Version 1.8 adds `selectedTrackStore`, `513008` live-tail appending after `513009` history load, provisional-point replacement, configurable `120s` coverage-gap continuity, valid-waypoint interrupted gap connectors, known-destination planned route links, and exported continuity diagnostics.
...
Open `admin.html` to manage aircraft icon mappings in the 1.8 control console.
